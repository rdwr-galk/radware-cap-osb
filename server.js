...
async function createApp() {
  try {
    ...
    // 2) Initialize store based on configuration
    let store;
    ...
    // 3) Build express app (unchanged logic, just moved after config load)
    app = express();
    ...
    app.use((err, req, res, next) => {
      if (err && err.type === 'entity.too.large') {
        logger.warn({ correlationId: req.correlationId, error: err.message }, 'Payload too large');
        return res.status(413).json({ description: 'Payload too large' });
      }
      if (err && err instanceof SyntaxError && 'body' in err) {
        logger.warn({ correlationId: req.correlationId, error: err.message }, 'Invalid JSON payload');
